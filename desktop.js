...
fetch(userRecomendation)
  ...
  .then(data => {
    ...
    trendingInd.forEach(trendingInd=>{
      genereateHT += generateImageHTML(trendingInd.image, trendingInd.songname, trendingInd.artist, trendingInd.id, trendingInd.song); 
    })
    songs.forEach(song => {
        genereateHTML += generateImageHTML(song.image, song.songname, song.artist, song.id, song.song); 
    });
    ...
    <div class="inside-container-element">
      <div class="show-1">
        <img
          ...
          alt=""
        />
        <div class="icons-play" data-song-id='${id}'>
...
function getIndex(indx){
  ...
  const findIdx = document.querySelectorAll('.icons-play');
...
if (!isElementFullscreen()) {
  ...
  changeBackgroundImage(`${currentSong.image}`);
    ...
    function setSongProgress(event) {
      const clickedPositionX = event.clientX - progressBarContainer.getBoundingClientRect().left;
...
function togglePlayPauseForSong(songId) {
    ...
    if (index !== -1) {
        const playPauseBtn = document.querySelector(`.icons-play[data-song-id="${songId}"] .play-pause-main`);
        console.log(playPauseBtn)
        // Toggle play/pause state for the current song
        songStates[songId] = !songStates[songId];
        
        if (songStates[songId]) {
            playPauseBtn.innerHTML = '<img src="https://img.icons8.com/ios-filled/100/FFFFFF/circled-pause.png"/>';
            currentSongIndex = index;
            updateSongInfo();
            playSong();
            togglePlayPause();
        } else {
            playPauseBtn.innerHTML = '<img src="https://img.icons8.com/ios-filled/100/FFFFFF/circled-play.png"/>';
            pauseSong();
            updateSongInfo();
            togglePlayPause();
        }
...
const playPauseButtons = document.querySelectorAll('.icons-play');
